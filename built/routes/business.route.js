...
businessRoutes.route("/").get(function (req, res) {
    Business_1.default.find(function (err, businesses) {
        if (err) {
            console.log(err);
        }
        else {
...
businessRoutes.route("/edit/:id").get(function (req, res) {
    ...
    Business_1.default.findById(id, function (err, business) {
        res.json(business);
    });
});
//  Defined update route
businessRoutes.route("/update/:id").post(function (req, res) {
    Business_1.default.findById(req.params.id, function (err, next, business) {
        if (!business) {
...
businessRoutes.route("/delete/:id").get(function (req, res) {
    Business_1.default.findByIdAndRemove({ _id: req.params.id }, function (err, business) {
        if (err) {
            res.json(err);
        }
        else {
